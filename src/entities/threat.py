...
class Threat(Entity):
    ...
    def __init__(self, shape: Shape = Shape.CIRCLE, radius=10, max_radius=500, color=(0, 0, 0), mode=GrowthMode.LINEAR,
        ...
        self.mode = mode
        self.shape = shape
        self.width = self.initial_radius
        self.height = self.initial_radius
    ...
    def draw_entite(self) -> pygame.Surface:
        ...
        if self.shape == Shape.CIRCLE:
            pygame.draw.circle(surface, self.color, center, int(self.initial_radius // 2))
            return surface
        else:
    ...
    def move(self):
        ...
        self.radius = self.max_radius * compute_growth(self.local_timer // self.ANIMATION_TIME, self.mode, self.a,
                                                       self.b)
